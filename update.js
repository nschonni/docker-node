...
let nodeVersionData;
getMyFile(nodeVersionsUrl)
  .then(response => {
    nodeVersionData = response
    updateImages()
  })
  .catch(error => {
    // handle error here
...
function updateImages() {
  ...
  for (version of versions) {
    ...
    for (variant in variants) {
      ...
      if (isAlpine) {
        getMyFile(`https://unofficial-builds.nodejs.org/download/release/v${currentVersion}/SHASUMS256.txt`)
          .then(response => {
            let checkSumRE = new RegExp(`(?<checksum>\\w+)  node-v${currentVersion}-linux-x64-musl.tar.xz`)
            let checkSum = checkSumRE.match(response).groups.checksum
            console.log(checkSum)
            dockerfile = dockerfile.replace('CHECKSUM_x64', `${checkSum}`)
